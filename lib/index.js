...
export default class CsvToJson extends Activity {
    ...
    onTask(activityTask, config) {
        return CsvToJson.convertCsvToJson(activityTask.input.file, config);
    }
     ...
     */
    static convertCsvToJson(stream, config) {
        const converter = CsvToJson.buildConverter(config);
        return stream.pipe(converter);
    }

    static buildConverter(config) {
